...
for file in files:
    ...
    # Parse raw timestamp string to standard publication format
    if len(timestamp) >= 12:
        formatted_ts = f"{timestamp[0:4]}-{timestamp[4:6]}-{timestamp[6:8]} {timestamp[8:10]}:{timestamp[10:12]} UTC"
    else:
    ...
    # CRITICAL CLEANUP: Mask out model prediction artifacts over land 
    land_mask = np.isnan(true_speed) | (true_speed == 0)
    
    # Optional: Apply subtle Gaussian smoothing to the ML prediction to remove 
    # high-frequency checkerboard noise before masking
    pred_speed = gaussian_filter(pred_speed, sigma=0.5)
    
    pred_speed[land_mask] = np.nan
    true_speed[land_mask] = np.nan

    # Calculate absolute error over ocean cells only
    abs_error = np.abs(pred_speed - true_speed)
    abs_error[land_mask] = np.nan

    # Establish uniform colorbar ceilings matching maximum observation bounds
    vmax = 0.35  # Standardized ceiling for this region's current dynamics
    vmax_error = 0.10
    ...
    # Standardize map limits, aspect ratios, and grid labels across panels
    for i, ax in enumerate(axes):
        ax.set_extent([LON_MIN, LON_MAX, LAT_MIN, LAT_MAX], crs=ccrs.PlateCarree())
        
        # Use a professional light gray for land to provide clear contrast with white missing data bounds
        ...
        # Keep y-axis coordinates visible only on the leftmost frame to optimize horizontal space
        if i > 0:
            gl.left_labels = False

    # Define robust scientific colormaps
    ...
    # Quantization levels for smooth vector graphics contour generation
    speed_levels = np.linspace(0, vmax, 35)
    ...
    axes[0].set_title("(A) Actual HYCOM Surface Currents", fontsize=13, fontweight="bold", pad=12)
    ...
    axes[1].set_title("(B) GOFLOW Prediction Pattern", fontsize=13, fontweight="bold", pad=12)

    # --- PANEL 3: Absolute Spatial Error ---
    im3 = axes[2].contourf(
        lons, lats, abs_error,
        levels=error_levels,
        cmap=cmap_error,
    ...
    axes[2].set_title("(C) Absolute Prediction Error", fontsize=13, fontweight="bold", pad=12)

    # ==========================================================================
    # FIXED BALANCED COLORBAR GRID PLACEMENT 
    # ==========================================================================
    # Assigning individual horizontal colorbars to each specific subplot
    # forces uniform aspect ratio balancing across all three panel images.
    ...
    # ==========================================================================
    # EXPORT CONTROL & LAYOUT SPACE ALLOCATION
    # ==========================================================================
    plt.suptitle(f"GO FLOW V2 (9KM) Validation Analysis — {formatted_ts}", fontsize=15, fontweight="bold", y=0.96)

    # 1. tight_layout cleanly scales standard figure elements
    plt.tight_layout()
    # 2. subplots_adjust overrides layout boundaries to build an absolute safe margin for headers
    ...
    plt.close()
    print("Saved Perfectly Balanced Publication Layout:", outfile)
