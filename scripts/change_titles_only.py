...
print("\nUsing existing prediction arrays only.")
...
for event in EVENTS:
    ...
    file_path = os.path.join(
        PRED_ARRAY_DIR,
        f"prediction_{event}.npz"
    )

    if not os.path.exists(file_path):
        print(f"Missing prediction file: {event}")
        continue

    d = np.load(file_path)

    true_speed = d["true_speed"]
    pred_speed = d["pred_speed"]
    err_speed = d["error_speed"]

    land_mask = np.isnan(true_speed)

    true_speed[land_mask] = np.nan
    pred_speed[land_mask] = np.nan
    err_speed[land_mask] = np.nan

    true_plot = gaussian_nan_smooth(
        true_speed,
        sigma=0.5
    )

    pred_plot = gaussian_nan_smooth(
        pred_speed,
        sigma=0.5
    )

    err_plot = gaussian_nan_smooth(
        err_speed,
        sigma=0.5
    )
